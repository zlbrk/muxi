...
%Записывем граничные условия и проверяем, что они заполнены
%---------------------------------------
[iCFI uCFI vCFIPTS CFIPTS] = mkCFI()

if  numel([POINTS.id]) ~= sum([CFIPTS.numPTS])
    fprintf('Граничные условия установлены не на все стороны!\n');
    return
end

if  CATS.Zc ~= POINTS(2).Z
    fprintf('Координты катода не совподают с координатами контура!\n');
    return
end
